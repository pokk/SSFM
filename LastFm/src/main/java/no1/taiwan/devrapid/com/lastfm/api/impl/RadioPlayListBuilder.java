...
import no1.taiwan.devrapid.com.lastfm.api.RadioTrack;
...
public class RadioPlayListBuilder extends XMLBuilder<RadioPlayList> {
    ...
    public RadioPlayList build(Node radioTracklistNode) {
        ...
        if (extensionNode != null) {
            expired = !XMLUtil.findNamedElementNode(extensionNode, "expired")
                              .getFirstChild()
                              .getNodeValue()
                              .contentEquals("0");
            playsLeft = Integer.parseInt(XMLUtil.findNamedElementNode(extensionNode, "playsleft")
                                                .getFirstChild()
                                                .getNodeValue());
